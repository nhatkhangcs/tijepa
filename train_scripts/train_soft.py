...
from src.utils.losses import similarity_matrix, contrastive_loss, clip_loss, semantic_soft_clip_loss
                ...
                for i in range(0, len(images), mini_batch_size):
                    ...
                    with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=True):
                        ...
                        with torch.no_grad():
                            ...
                            text_repr = encoded_text[:, 0, :].to(DEVICE_0)
                            encoded_image_full = vision_encoder(mini_images)  # Encode the context patches
                            image_repr = encoded_image_full.mean(dim=1)
                        ...
                        representation_predicted = predicted.mean(dim=1)
                        representation_target = target.mean(dim=1)
                        ...
                        # print_tensor_with_precision(representation_target[1][:10])

                        c_loss = semantic_soft_clip_loss(
                            text_repr, 
                            image_repr,
                            representation_predicted, 
                            representation_target, 
                            temperature=0.3
                        )
                        # print(f"{c_loss.tolist()=}")
                        loss = c_loss + 10 * p_loss
                        ...
                        saver.update_metric(
                            {
                                ...
                                # 'mem0': torch.cuda.max_memory_allocated(DEVICE_0) / 1024.**3,
                                # 'mem1': torch.cuda.max_memory_allocated(DEVICE_1) / 1024.**3
                            }
                        )
...
def main():
    train(
        ...
        mini_batch_size=80, # 80
        batch_size=80*5, # 80*6
        learning_rate=0.003,
        save_interval=1,
        resume_from=None,
    )
