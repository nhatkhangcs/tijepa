                ...
                for i in range(0, len(images), mini_batch_size):
                    ...
                    with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=True):
                        ...
                        with torch.no_grad():
                            ...
                            encoded_image_full = vision_encoder(mini_images).to(DEVICE_1)  # Encode the context patches
...
def main():
    train(
        ...
        mini_batch_size=65, # 80
        batch_size=65*50, # 80*6
        learning_rate=0.003,
        save_interval=1,
        resume_from=None,
    )
