...
from src.utils.visualizer import visualize_rectangle, print_tensor_with_precision
...
def train(num_epochs=1, max_images_per_epoch=10, batch_size=10, mini_batch_size=10, learning_rate=0.01):
        ...
        with tqdm(dataset, desc=f"Epoch {epoch+1}/{num_epochs}") as pbar:
            for images, captions, context_masks, predict_masks in pbar:
                ...
                for i in range(0, len(images), mini_batch_size):
                    ...
                    mini_predict_masks = mini_predict_masks.to(DEVICE_0, non_blocking=True)
                    ...
                    print_tensor_with_precision(predicted[1][0][:10])
                    print()
                    print_tensor_with_precision(target[1][1][:10])
                    print_tensor_with_precision(predicted[1][1][:10])
                    print()

                    # print those above list with 4 number precision
                    # print those above list with 4 number precision
