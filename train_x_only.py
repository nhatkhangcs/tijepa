    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(dataset, desc=f"Epoch {epoch+1}/{num_epochs}") as pbar:
            for images, captions, context_masks, predict_masks in pbar:
                ...
                encoded_image = vision_encoder(images)  # Encode the context patches
                masked_image = apply_masks(encoded_image, context_masks)  # Apply context mask

                encoded_text = encoded_text.to(DEVICE_1)
                text_attn_mask = text_attn_mask.to(DEVICE_1)
                encoded_image = encoded_image.to(DEVICE_1)
                masked_image = masked_image.to(DEVICE_1)
                context_masks = context_masks.to(DEVICE_1)
                predict_masks = predict_masks.to(DEVICE_1)
                ...
                cross_encoded = context_crosser(encoded_text, masked_image, text_attn_mask)  # Cross encode the text and context
                # print(f"{cross_encoded.shape=}")
                predicted = predictor(cross_encoded, context_masks, predict_masks)  # Generate predictions based on context
                # print(f"{predicted.shape=}")
                
                cross_encoded_target = target_crosser(encoded_text, encoded_image, text_attn_mask)  # Cross encode the text and context
